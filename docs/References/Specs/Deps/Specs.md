...
- `deps.go` must not import anything from `adapters/`, `examples/libraryExamples/`, `tests/`, `sandbox/lib/`, `sandbox/contracts/api`, or `sandbox` (the entry point) — the contract stays free of implementations.
