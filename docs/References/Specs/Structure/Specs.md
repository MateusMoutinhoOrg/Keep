...
3. **One `##` section per top-level directory** (`Root`, `/adapters/`, `/docs/`, `/examples/libraryExamples/`, `/pkg/`), each with a `File | Description | Spec` table (or nested subsection tables), separated by `---`.
