...
- An example may import `adapters/<name>` and `sandbox`; it must never reconstruct dependencies by hand — that is the adapter's job.
...
2. **Imports**: an adapter (e.g. `adapters/standard`) and `sandbox`.
