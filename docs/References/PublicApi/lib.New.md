...
Injects a [`deps.Deps`](./deps.Deps.md) implementation into the library and returns the [`api.Lib`](./api.Lib.md) entry point. It is the single wiring point consumers touch, and the only exported symbol of the `sandbox` package. It delegates to `sandbox/internal/lib.New`, which stores the deps on the struct and runs every field factory over it.
