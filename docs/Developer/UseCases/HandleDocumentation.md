...
1. Identify where the document belongs:
   - **Consumer-facing** docs → `docs/Consumer/` (or `docs/Consumer/UseCases/` for use cases).
   - **Developer-facing** docs → `docs/Developer/` (or `docs/Developer/UseCases/` for use cases).
...
5. Update the [README.md](../../../README.md) Internal Docs section (Consumer or Developer table).
...
### Workflow
1. Create the `.md` file inside the relevant `UseCases/` directory with a topic-based name that groups related actions (e.g., `HandleDependencies.md`).
