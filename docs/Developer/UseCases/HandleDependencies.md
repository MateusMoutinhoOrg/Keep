...
2. Add a new descriptive field to the `Deps` struct for the required function:
   ...
   ```
3. If the function must distinguish an expected condition (e.g., a missing key) from a real failure, add a sentinel error next to the existing ones in the same file, and document when to return it in [RequiredApi.md](../../Consumer/RequiredApi.md).
...
5. Document the new function's contract in [RequiredApi.md](../../Consumer/RequiredApi.md) and update [deps.Deps.md](../../Consumer/PublicApi/deps.Deps.md).
...
2. Define a `New()` factory that constructs and returns a `deps.Deps`, implementing every field required by the `Deps` struct in [pkg/deps/deps.go](../../../pkg/deps/deps.go) and honoring the contract in [RequiredApi.md](../../Consumer/RequiredApi.md):
   ```go
   package myadapter
