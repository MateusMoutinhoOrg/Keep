...
- A page can belong to one or more **themes** — `LibUsage`, `Development`, `Templating` — and its themes are expressed by the indexes that list it, never by the page's location.
...
6. Add an entry to the document's index:
   ...
   - For subdirectory pages: add it to [PublicApi.md](/docs/References/PublicApi.md) or [Specs.md](/docs/References/Specs.md) respectively.
...
4. Update the document's entry in its theme index under `docs/Index/` — the link, the description, and every topic-section link.
...
4. Remove the document's entry from its theme index under `docs/Index/`.
