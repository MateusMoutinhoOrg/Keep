...
The closure captures `s`, so `s.Deps.Exists(...)` is resolved when `CheckKeysPresence` is *called*, not when the factory ran. That is what carries the injected deps into behavior the caller can hold, while `sandbox/internal/` stays unreachable from outside.
