...
`{c}` is the collection prefix — `Props.Path` + the schema name for a top-level collection,
`{c}-{id}-{field}` for one nested inside a record. The two are the same thing: every key
family below applies unchanged to a nested collection.
...
case-insensitive, and hashed at all so that key length stays bounded and no character of a
value can collide with the layout's separator.
