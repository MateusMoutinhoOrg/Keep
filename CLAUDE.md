...
```
adapters/standard  ──constructs──▶  deps.Deps  ──injected into──▶  keep.New(deps)
...
- **`pkg/keep/`** — the entry point: `KeepLib` holds the injected `Deps` and hands out databases via `NewDatabase(props)`.
- **`pkg/database/`** — the engine: `KeepDatabase` → `SchemaInstance` (a collection) → `SchemaItem` (a record). Each object carries the injected `Deps`, directly or through a back-pointer to its owner. Never imports `adapters/`.
...
| add a lib function/object | `docs/Tutorials/AddLibFunction.md`, `AddLibObject.md` | `docs/Reference/PublicApi.md` (+ detail page in `docs/Reference/PublicApi/`, see `ExposePublicApi.md`) |
...
- Operations return a typed `*database.Error`; switch on its `Type` rather than matching strings — see `docs/Reference/Errors.md`.
