...
`AGENTS.md` carries the four standing rules every change must respect, and `docs/References/Specs.md` is the index of every file specification — each spec's own `### Rules` section is where a file's binding rules live. Adding a `Deps` field without implementing it in all adapters breaks every consumer — that's the most common footgun. A new public lib function or object must be declared in `sandbox/contracts/api/api.go` **and** implemented (as factories) in `sandbox/lib/`, or callers cannot reach it.
